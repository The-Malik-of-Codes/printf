...
int width, int precision, int size)
{
	...
	int length = (str == NULL) ? handle_null_string(&str, precision) :
	(handle_valid_string(str, precision));
...
int width, int precision, int size)
{
	...
	unsigned long int num = (is_negative) ? (unsigned long int)(-n) :
	(unsigned long int)n;

	int length = handle_number_length(num);

	int padded_length = handle_padded_length(width, precision, is_negative,
	length);

	return (handle_width_padding(num, buffer, flags, width, padded_length,
	precision));
}
...
int width, int precision, int size)
{
	...
	return (handle_width_padding(n, buffer, flags,
	width, padded_length, precision));
}
